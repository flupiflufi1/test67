...
async def _handle_client(reader, writer):
    ...
    try:
        ...
        # Android (and possibly iOS) with useSecret=0 has random dc_id bytes — patch it.
        # Also covers the case where _dc_from_init fails entirely.
        if dc is None and dst in _IP_TO_DC:
            dc, is_media = _IP_TO_DC[dst]
            log.debug("[%s] DC from IP map: DC%d is_media=%s", label, dc, is_media)
            # Always patch the init packet when DC came from IP map,
            # regardless of whether this DC is in _dc_opt.
            init = _patch_init_dc(init, dc if is_media else -dc)
            init_patched = True

        if dc is None:
            log.warning("[%s] cannot determine DC for %s:%d -> TCP passthrough",
                        label, dst, port)
            await _tcp_fallback(reader, writer, dst, port, init, label)
            return

        # If this DC is not in _dc_opt, fall back to using the destination IP
        # directly as the WS target (best-effort).
        if dc not in _dc_opt:
            log.info("[%s] DC%d not in config, using dst IP %s as WS target",
                     label, dc, dst)
            _dc_opt[dc] = dst
        ...
        domains = _ws_domains(dc, is_media)
        target = _dc_opt[dc]
        # If the configured target differs from the actual destination,
        # also try the destination IP as a fallback WS target.
        extra_targets = [target]
        if dst != target and _is_telegram_ip(dst):
            extra_targets.append(dst)
        ws = None
        ...
        if ws:
            ...
            for try_target in extra_targets:
                if ws:
                    break
                for domain in domains:
                    url = f'wss://{domain}/apiws'
                    log.info("[%s] DC%d%s (%s:%d) -> %s via %s",
                             label, dc, media_tag, dst, port, url, try_target)
                    try:
                        ws = await RawWebSocket.connect(try_target, domain,
                                                        timeout=ws_timeout)
                        all_redirects = False
                        if try_target != target:
                            log.info("[%s] DC%d%s connected via fallback target %s",
                                     label, dc, media_tag, try_target)
                        break
                    except WsHandshakeError as exc:
                        _stats.ws_errors += 1
                        if exc.is_redirect:
...
def parse_dc_ip_list(dc_ip_list: List[str]) -> Dict[int, str]:
    ...
    Also accepts plain IP addresses (without DC: prefix) — the DC number
    will be auto-detected from the known _IP_TO_DC map.
    """
    dc_opt: Dict[int, str] = {}
    for entry in dc_ip_list:
        ...
        if not entry:
            continue
        if ':' not in entry:
            # Plain IP — try to auto-detect DC from known map
            ip_s = entry
            dc_n = None
            for ip, (dc_id, is_media) in _IP_TO_DC.items():
                if ip == ip_s and not is_media:
                    dc_n = dc_id
                    break
            if dc_n is None:
                raise ValueError(
                    f"Unknown IP {entry!r} — cannot auto-detect DC. "
                    f"Use format DC:IP, e.g. '2:{entry}'"
                )
            try:
                _socket.inet_aton(ip_s)
            except OSError:
                raise ValueError(f"Invalid IP in --dc-ip {entry!r}")
            dc_opt[dc_n] = ip_s
        else:
